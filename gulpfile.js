...
gulp.task('script', function () { //аналогично поступаем с js-файлами
  return gulp.src([ //тут подключаем разные js в общую библиотеку. Отключите то, что вам не нужно.
      ...
      'src/js/jquery-ui-1.8.23.custom.min.js',
      'src/js/jquery.kinetic.js',
      'src/js/jquery.mousewheel.min.js',
      'src/js/jquery.smoothdivscroll-1.3-min.js',
      'src/js/swiper.js',
      'src/js/vanilla-picker.js'
    ])
    .pipe(size())
    .pipe(sourcemaps.init())
    .pipe(babel())
...
//   return gulp.src(['src/**/*.*', '!src/components/**/*.*', '!src/fonts/**/*.*', '!src/img/**/*.*', '!src/js/jquery-3.2.1.js', '!src/scss/**/*.*', '!src/**/*.html'])
//   .pipe(gulp.dest('build'))
// })
